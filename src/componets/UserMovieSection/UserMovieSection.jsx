...
function UserMovieSection(props) {
  ...
  function getMovies() {
    setIsLoading(true);
    getDoc(doc(db, props.from, User.uid))
      .then((result) => {
        if (result.exists()) {
          const mv = result.data();
          // Check if mv.movies exists before trying to use it
          if (mv && mv.movies) {
            setMyMovies(mv.movies);
            // Only set isResultEmpty if the array is empty
            setIsResultEmpty(mv.movies.length === 0);
          } else {
            // If mv.movies doesn't exist, treat as empty
            setMyMovies([]);
                ...
                <div className="p-1 mt-2 mb-5">
                  ...
                  >
                      <img
                        onClick={() => handleMoviePopup(movie)}
                        className=""
                        src={movie.backdrop_path ? imageUrl2 + movie.backdrop_path : "https://i.ytimg.com/vi/Mwf--eGs05U/maxresdefault.jpg"}
                      ...
                      <div className="mt-1">
                        {converted &&
                          converted.map((genre, index) => {
                            return (
                              <span key={index} className="text-white mr-4 text-xs 2xl:text-sm font-thin">
                                {genre}
                              </span>
                            );
                          })}
